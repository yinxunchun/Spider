...
public class BJWB implements NEWSPAPER{
	...
	public void getBJWB(){
		...
		String[] timeLabel = new String[]{"width","316"};   
		String[] sourceLabel =new String[]{"北京晚报","北京晚报：http://bjwb.bjd.com.cn/"}; 
		String[] categroyLabel = new String[]{"width","145"} ; 
		...
		//新闻入口赋值
		themeUrl = "http://bjwb.bjd.com.cn/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_82.htm";

		//主题连接 赋值
		themeLinksReg = "http://bjwb.bjd.com.cn/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_[0-9]{1,3}.htm";
...
//			titleBuf = titleBuf ;
	...
	public String getNewsContent(String html, String[] label) {
		...
		if(html!=null&&html.contains("<!--enpcontent-->")&&html.contains("<!--/enpcontent-->")){
			contentBuf = html.substring(html.indexOf("<!--enpcontent-->")+17, html.indexOf("<!--/enpcontent-->"));
	...
	public String getNewsImages(String html, String[] label) {
		if(html == null)
			return null;
		String bufHtml;
		if(html.contains("<!--enpcontent-->")&&html.contains("<table bgcolor=#efefef>")){
			bufHtml = html.substring(html.indexOf("<table bgcolor=#efefef>"), html.indexOf("<!--enpcontent-->"))  ;      //辅助
		}else {
			return null;
		}        //辅助
...
//		System.out.println(imageNameTime);
    	...
    	//图片正则表达式
		String imageReg = "../../../images/"+imageNameTime.substring(0, 4)+"-"+imageNameTime.substring(4,6)+"/"+imageNameTime.substring(6, 8)+"/[0-9]{2}/(.*?).jpg";
...
//			System.out.println(bufUrl);
			bufUrl =  "http://bjwb.bjd.com.cn/" + bufUrl;
			File fileBuf;
//			imageMatcher.group();
			...
			try{
				...
				FileOutputStream fo;
				if(imageNumber < 10){
					fileBuf = new File("BJWB",imageNameTime+photohour+photominute+photosecond+"000"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf); 
					imageLocation.offer(fileBuf.getPath());
				}else if(imageNumber < 100){
					fileBuf = new File("BJWB",imageNameTime+photohour+photominute+photosecond+"00"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
            
				}else if(imageNumber < 1000){
					fileBuf = new File("BJWB",imageNameTime+photohour+photominute+photosecond+"0"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
  
				}else{
					fileBuf = new File("BJWB",imageNameTime+photohour+photominute+photosecond+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
				}
