...
//新闻晨报
public class XWCB implements NEWSPAPER{
	...
	public void getXWCB(){
		...
		String[] titleLabel = new String[]{"h1",""};     
		String[] contentLabel = new String[]{"class","content"};  
		String[] timeLabel = new String[]{"h5",""};   
		String[] sourceLabel =new String[]{"新闻晨报","新闻晨报：http://www.jfdaily.com/shouye/jishi/","h5"}; 
		String[] categroyLabel = new String[]{"h5",""} ; 
		...
		//新闻入口赋值
		themeUrl = "http://newspaper.jfdaily.com/xwcb/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_2.htm";

		//主题连接 赋值
		themeLinksReg = "http://newspaper.jfdaily.com/xwcb/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_[0-9]{1,2}.htm";
		
		//内容连接 赋值
		contentLinksReg = "http://newspaper.jfdaily.com/xwcb/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/"+"content_[0-9]{5,7}.htm";
...
//        System.out.println(html);
	...
	public String getNewsTitle(String html, String[] label, String buf) {
		...
		if(titleBuf!=null&&titleBuf.contains("\n                                                    "))
			titleBuf = titleBuf.replace("\n                                                    ", "");
		return titleBuf;
	}
	...
	public String getNewsImages(String html, String[] label) {
		if(html == null)
			return null;
		String bufHtml = null ;
		if(html.contains("<div id='coin-slider'>")&&html.contains("<div class=\"content\">")){
			bufHtml = html.substring(html.indexOf("<div id='coin-slider'>"),html.indexOf("<div class=\"content\">")) ;        //辅助
		}
...
//		System.out.println(imageNameTime);
		...
		String imageReg = "src=(.*?)xwcb\\\\resfiles\\\\[0-9]{4}-[0-9]{2}\\\\[0-9]{2}/(.*?).jpg";
...
//			System.out.println(bufUrl);
			...
			bufUrl = bufUrl.replaceAll("src=\"..\\\\..\\\\..\\\\..\\\\", "");
			bufUrl = bufUrl.replaceAll("\\\\", "/");
//			System.out.println(bufUrl);
			bufUrl =  "http://newspaper.jfdaily.com/" + bufUrl;
			File fileBuf;
//			imageMatcher.group();
...
//          	  System.out.println(imageName + "下载完成"); 
	...
	public String getNewsOriginalSource(String html, String[] label) {
		String bufNewsSource = null;
		if(label.length == 3 && (!label[2].equals(""))){
			bufNewsSource = getHtml(html, label[2]);
			if(bufNewsSource!=null&&bufNewsSource.contains("来源：")){
				bufNewsSource = bufNewsSource.substring(bufNewsSource.indexOf("来源："), bufNewsSource.length());
				bufNewsSource = bufNewsSource.replaceAll("\\s+","");
			
			}
			return label[0]+"-"+bufNewsSource;		
		}	
		else
			return label[0];
	}
	...
	public String getNewsCategroy(String html, String[] label) {
		...
		if(categroyBuf!=null){
			if(categroyBuf.contains("：")&&categroyBuf.contains("稿件来源："))
			 categroyBuf = categroyBuf.substring(categroyBuf.indexOf("：")+1, categroyBuf.indexOf("稿件来源：")-1);
	...
	public String getNewsOriginalCategroy(String html, String[] label) {
		...
		if(categroyBuf!=null){
			if(categroyBuf.contains("日")&&categroyBuf.contains("稿件来源")){
				categroyBuf = categroyBuf.substring(categroyBuf.indexOf("日")+1, categroyBuf.indexOf("稿件来源")-1);
			}
			categroyBuf = categroyBuf.replaceAll("\\s+", "");
		}
		return categroyBuf;
