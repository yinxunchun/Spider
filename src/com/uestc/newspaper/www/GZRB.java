...
public class GZRB implements NEWSPAPER{
	...
	public void getGZRB(){
		System.out.println("gzrb start...");
		DBName = "NEWSPAPER";
		DBTable ="GZRB";
		ENCODE = "utf-8";
		
		String[] titleLabel = new String[]{"h3",""};     
		String[] contentLabel = new String[]{"class","article"};  //特殊处理
		String[] timeLabel = new String[]{"class","header-today"};  //特殊 需要根据日期定义  
		String[] sourceLabel =new String[]{"广州日报","广州日报：http://gzdaily.dayoo.com/"}; 
		String[] categroyLabel = new String[]{"title",""} ; 
		...
		//新闻入口赋值
		themeUrl = "http://gzdaily.dayoo.com/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_1.htm";

//		//主题连接 赋值
//		themeLinksReg = "http://e.chengdu.cn/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_[0-9]{1,3}.htm";
		
		//内容连接 赋值
		contentLinksReg = "http://gzdaily.dayoo.com/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/content_[0-9]{7,8}.htm";
...
//		System.out.println(cdsbContentQueue);
		...
		while(!cdsbContentQueue.isEmpty()){
			...
			if(!crut.query("Url", url)){
				...
				if(html!=null)
					crut.add(getNewsTitle(html,titleLabel,"广州日报"), getNewsOriginalTitle(html,titleLabel,"广州日报"),getNewsOriginalTitle(html,titleLabel,"广州日报"), getNewsTime(html,timeLabel),getNewsContent(html,contentLabel), getNewsSource(html,sourceLabel),
...
//			titleBuf = titleBuf.substring(0, titleBuf.indexOf(buf))	;
	...
	public String getNewsContent(String html, String[] label) {
		...
		if(html!=null&&html.contains("<founder-content>")&&html.contains("</founder-content>")){
			contentBuf = html.substring(html.indexOf("<founder-content>")+17,html.indexOf("</founder-content>"));
			if(contentBuf!=null){
	...
	public String getNewsImages(String html, String[] label) {
		if(html == null)
			return null;
		String bufHtml = null ;
		if(html.contains("<div id=\"leftMedia\" class=\"textleft\">")&&html.contains("<founder-content>"))
			bufHtml = html.substring(html.indexOf("<div id=\"leftMedia\" class=\"textleft\">"), html.indexOf("<founder-content>"));       //辅助
		else {
			bufHtml = html ;
		}
...
//		System.out.println(imageNameTime);
    	...
    	//图片正则表达式
		String imageReg = "../../../res/(([0-9]{1})|([0-9]{1}/[0-9]{1}))/"+imageNameTime.substring(0, 4)+"-"+imageNameTime.substring(4, 6)+"/"+imageNameTime.substring(6, 8)+"/(.*?)/res[0-9]{2}_attpic_brief.((jpg)|(JPG))";
...
//			System.out.println(bufUrl);
			bufUrl =  "http://gzdaily.dayoo.com/" + bufUrl;
			File fileBuf;
//			imageMatcher.group();
			...
			try{
				...
				FileOutputStream fo;
				if(imageNumber < 10){
					fileBuf = new File("GZRB",imageNameTime+photohour+photominute+photosecond+"000"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf); 
					imageLocation.offer(fileBuf.getPath());
				}else if(imageNumber < 100){
					fileBuf = new File("GZRB",imageNameTime+photohour+photominute+photosecond+"00"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
            
				}else if(imageNumber < 1000){
					fileBuf = new File("GZRB",imageNameTime+photohour+photominute+photosecond+"0"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
  
				}else{
					fileBuf = new File("GZRB",imageNameTime+photohour+photominute+photosecond+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
				}
...
//          	  System.out.println(imageName + "下载完成"); 
	...
	public String getNewsCategroy(String html, String[] label) {
		String bufCategroy =null;
		if(html!=null&&html.contains("<title>")&&html.contains("</title>"))
			bufCategroy = html.substring(html.indexOf("<title>")+7, html.indexOf("</title>"));
		if(bufCategroy!=null && bufCategroy.contains(":")&& bufCategroy.contains("-"))
			bufCategroy = bufCategroy.substring(bufCategroy.indexOf(":")+1, bufCategroy.lastIndexOf("-"));
		return bufCategroy;
	}
