...
public class NANDU implements NEWSPAPER{	
	...
	public void getNANDU(){
		...
		//新闻入口赋值
		themeUrl = "http://epaper.nandu.com/epaper/A/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_2731.htm";

		//主题连接 赋值
		themeLinksReg = "http://epaper.nandu.com/epaper/A/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_[0-9]{4}.htm";
		
		//内容连接 赋值
		contentLinksReg = "http://epaper.nandu.com/epaper/A/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/content_[0-9]{7,8}.htm\\?div=((0)|(-1)|(1))";
...
//			imageMatcher.group();
			...
			try{
				...
				FileOutputStream fo;
				if(imageNumber < 10){
					fileBuf = new File("NANDU",imageNameTime+"000"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf); 
					imageLocation.offer(fileBuf.getPath());
				}else if(imageNumber < 100){
					fileBuf = new File("NANDU",imageNameTime+"00"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
            
				}else if(imageNumber < 1000){
					fileBuf = new File("NANDU",imageNameTime+"0"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
  
				}else{
					fileBuf = new File("NANDU",imageNameTime+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
				}
