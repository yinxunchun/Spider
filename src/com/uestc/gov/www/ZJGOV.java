...
//浙江
public class ZJGOV implements GOV{
	...
	public void getZJGOVNews(){
		System.out.println("ZJ start...");
		DBName = "GOV";
		DBTable = "ZJGOV";
		...
		String[] newsTimeLabel = new String[]{"width","100%"};   //新闻时间  
		String[] newsSourceLabel =new String[]{"width","100%","浙江人民政府网站"}; //（3个参数）新闻来源 同新闻时间"class","ep-time-soure cDGray" 再加上一个"网易新闻-国内新闻"
		String[] newsCategroyLabel = new String[]{"",""} ; //暂时没有
      ...
      //计算获取新闻的时间
		...
		//要闻首页link 
		themeLinks.offer("http://www.zhejiang.gov.cn/");
		// 内容link 正则http://www.zhejiang.gov.cn/art/2015/1/9/art_15774_1476109.html
		newsContentLinksReg = "http://www.zhejiang.gov.cn/art/"+year+"/"+month+"/"+date+"/"+"art_[0-9]{5}_[0-9]{6,8}.html";
...
//        System.out.println(html);
	...
	public String getNewsImages(String html, String[] label) {
    	...
    	//图片正则表达式
		String imageReg = "/picture/(.*?)[0-9]{1}/[0-9]{18,20}.jpg";
		...
		while(imageMatcher.find()){
			...
			bufUrl = "http://www.zj.gov.cn" + bufUrl ;
...
//			imageMatcher.group();
			...
			try{
				...
				FileOutputStream fo;
				if(imageNumber <10){
					fileBuf = new File("ZJGOV",imageNameTime+photohour+photominute+photosecond+"000"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf); 
					imageLocation.offer(fileBuf.getAbsolutePath());
				}else if(imageNumber < 100){
					fileBuf = new File("ZJGOV",imageNameTime+photohour+photominute+photosecond+"00"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getAbsolutePath());
            
				}else if(imageNumber < 1000){
					fileBuf = new File("ZJGOV",imageNameTime+photohour+photominute+photosecond+"0"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getAbsolutePath());
  
				}else{
					fileBuf = new File("ZJGOV",imageNameTime+photohour+photominute+photosecond+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getAbsolutePath());
				}
...
//          	  System.out.println(imageName + "下载完成"); 
	...
	public String getNewsTime(String html, String[] label) {
		String timeBuf ="";
		if(html.contains("发布时间")&&html.contains("字号：[")){
			timeBuf = html.substring(html.indexOf("发布时间"), html.indexOf("字号：["));
	...
	public String getNewsOriginalSource(String html, String[] label) {
		String sourceBuf;
		if(html.contains("发布机构：")&&html.contains("字号：[")){
			
			sourceBuf = html.substring(html.indexOf("发布机构：")+5, html.indexOf("字号：["));
			sourceBuf = sourceBuf.replaceAll("</span><span style=\"margin-left:20px\">","");
		}else
			sourceBuf = "";
