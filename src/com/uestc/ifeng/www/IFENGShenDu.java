...
/*获取凤凰网的深度新闻
 * url:http://news.ifeng.com/shendu/
 * 每日更新最新信我呢
 * */
public class IFENGShenDu implements IFENG{
	...
	public void getIFENGShenDuNews(){
		System.out.println("IFENGSHENDU start...");
		DBName = "IFENG";
		DBTable = "SD";
		ENCODE = "utf-8";
		...
		String[] newsSourceLabel =new String[]{"class","p_time","凤凰新闻-深度新闻"}; //（3个参数）新闻来源 同新闻时间
		...
		//深度新闻 首页链接
		theme = "http://news.ifeng.com/shendu/";
		//新闻主题links的正则表达式 保留后期使用
//		newsThemeLinksReg ;
		
		//新闻内容links的正则表达式  http://news.ifeng.com/a/20141009/42161279_0.shtml
		newsContentLinksReg = "http://news.ifeng.com/a/"+yearBuf+"[0-9]{4}/[0-9]{7,9}_0.shtml";
		//保存社会新闻主题links
		Queue<String> shenDuNewsTheme = new LinkedList<String>();
		shenDuNewsTheme.offer(theme);
...
//		System.out.println(guoNeiNewsContent);
		...
		int i = 0;
		while(!shenDuNewsContent.isEmpty()){
			String url = shenDuNewsContent.poll();
...
//	       	 	System.out.println(n.extractLink());
					...
					if(themeMatcher.find()){
						...
						if(!contentlinks.contains(n.extractLink())){
							if(!n.extractLink().contains("comment"))
...
//				imageMatcher.group();
				...
				try{
					...
					FileOutputStream fo;
					if(imageNumber < 10){
						fileBuf = new File("IFENGShenDu",imageNameTime+photohour+photominute+photosecond+"000"+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf); 
						imageLocation.offer(fileBuf.getPath());
					}else if(imageNumber < 100){
						fileBuf = new File("IFENGShenDu",imageNameTime+photohour+photominute+photosecond+"00"+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf);
						imageLocation.offer(fileBuf.getPath());
		            
					}else if(imageNumber < 1000){
						fileBuf = new File("IFENGShenDu",imageNameTime+photohour+photominute+photosecond+"0"+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf);
						imageLocation.offer(fileBuf.getPath());
		  
					}else{
						fileBuf = new File("IFENGShenDu",imageNameTime+photohour+photominute+photosecond+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf);
						imageLocation.offer(fileBuf.getPath());
					}
...
//		            System.out.println(imageName + "下载完成"); 
	...
	public static void main(String[] args){
		...
		IFENGShenDu test = new IFENGShenDu();
		test.getIFENGShenDuNews();
	}
}
