...
/*
 * 获取凤凰网军事新闻
 * 首页：http://news.ifeng.com/mil/
 * 分为：及时新闻 综合排行等等
 * 本程序在于获取最新的新闻，之前的新闻将在之后模块中考虑
...
public class IFENGMil implements IFENG{
	...
	public void getIFENGMilNews(){
		System.out.println("IFENGMIL start...");
		DBName = "IFENG";
		DBTable = "MIL";
		ENCODE = "utf-8";
		...
		String[] newsSourceLabel =new String[]{"class","p_time","凤凰新闻-军事新闻"}; //（3个参数）新闻来源 同新闻时间
...
//		System.out.println(i);
		crut.destory();
		System.out.println("IFENGMIL over...");
...
//				imageMatcher.group();
				...
				try{
					...
					FileOutputStream fo;
					if(imageNumber < 10){
						fileBuf = new File("IFENGMIL",imageNameTime+photohour+photominute+photosecond+"000"+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf); 
						imageLocation.offer(fileBuf.getPath());
					}else if(imageNumber < 100){
						fileBuf = new File("IFENGMIL",imageNameTime+photohour+photominute+photosecond+"00"+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf);
						imageLocation.offer(fileBuf.getPath());
		            
					}else if(imageNumber < 1000){
						fileBuf = new File("IFENGMIL",imageNameTime+photohour+photominute+photosecond+"0"+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf);
						imageLocation.offer(fileBuf.getPath());
		  
					}else{
						fileBuf = new File("IFENGMIL",imageNameTime+photohour+photominute+photosecond+imageNumber+"000"+i+imageNameSuffix);
						fo = new FileOutputStream(fileBuf);
						imageLocation.offer(fileBuf.getPath());
					}
