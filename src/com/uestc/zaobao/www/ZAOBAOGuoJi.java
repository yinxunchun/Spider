...
public class ZAOBAOGuoJi implements ZAOBAO{
	...
	public void getZAOBAOGuoJi(){
		System.out.println("ZAOBAOguoJI start...");
		DBName = "ZAOBAO";
		DBTable ="guoji";
		ENCODE = "utf-8";
		...
		String[] sourceLabel =new String[]{"联合早报-国际新闻","联合早报-国际新闻：http://www.zaobao.com/news/world"}; 
		String[] categroyLabel = new String[]{"国际新闻","联合早报-国际新闻"} ; 
		...
		//新闻入口赋值
		themeUrl = "http://www.zaobao.com/news/world";

		//主题连接 赋值
	//	themeLinksReg = "http://e.chengdu.cn/html/"+year+"-"+bufMonthString+"/"+bufDateString+"/node_[0-9]{1,3}.htm";
		
		//内容连接 赋值
		contentLinksReg = "http://www.zaobao.com/news/world/story"+year+bufMonthString+bufDateString+"-[0-9]{6}";
...
//		System.out.println(cdsbThemeQueue);
	...
	//	System.out.println(cdsbContentQueue);
		...
		System.out.println("zaobao guoji: test 3...");
		while(!cdsbContentQueue.isEmpty()){
			System.out.println("zaobao guoji: test 4...");
			String url = cdsbContentQueue.poll();
			if(!crut.query("Url", url)){
				System.out.println("zaobao guoji: test 5...");
				Date date = new Date();
				String html = getContentHtml(url);
				System.out.println("zaobao guoji: test 6...");
				if(html!=null){
					System.out.println("zaobao guoji: test 7...");
				...
				System.out.println("zaobao guoji: test 8...");
			}
		}
		crut.destory();
		System.out.println("zaobaoguoji over..");
	}
	@Override
...
//			imageMatcher.group();
			...
			try{
				...
				FileOutputStream fo;
				if(imageNumber < 10){
					fileBuf = new File("ZAOBAOGUOJI",imageNameTime+photohour+photominute+photosecond+"000"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf); 
					imageLocation.offer(fileBuf.getPath());
				}else if(imageNumber < 100){
					fileBuf = new File("ZAOBAOGUOJI",imageNameTime+photohour+photominute+photosecond+"00"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
            
				}else if(imageNumber < 1000){
					fileBuf = new File("ZAOBAOGUOJI",imageNameTime+photohour+photominute+photosecond+"0"+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
  
				}else{
					fileBuf = new File("ZAOBAOGUOJI",imageNameTime+photohour+photominute+photosecond+imageNumber+"000"+i+imageNameSuffix);
					fo = new FileOutputStream(fileBuf);
					imageLocation.offer(fileBuf.getPath());
				}
...
//		}
	...
	public static void main(String[] args){
		ZAOBAOGuoJi test = new ZAOBAOGuoJi();
		test.getZAOBAOGuoJi();
	}
}
